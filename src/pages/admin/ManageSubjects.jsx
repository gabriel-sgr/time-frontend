...
export default function ManageSubjects() {
  ...
  const fetchItems = async () => {
    try { const res = await getSubjects(); setItems(res.data); }
  ...
  const handleCreate = async (e) => {
    ...
    try { await createSubject({ name: newName.trim() }); setNewName(''); fetchItems(); }
    catch (err) { alert(err.response?.data?.message || 'Error'); }
  };

  const handleUpdate = async (id) => {
    if (!editName.trim()) return;
    try { await updateSubject(id, { name: editName.trim() }); setEditId(null); fetchItems(); }
    catch (err) { alert(err.response?.data?.message || 'Error'); }
  };

  const handleDelete = async (id) => {
    if (!confirm('Delete this item?')) return;
    try { await deleteSubject(id); fetchItems(); }
    catch (err) { alert(err.response?.data?.message || 'Error'); }
  };

  const handleToggleActive = async (id, currentStatus) => {
    try { 
      // Handle undefined is_active (treat as true for existing records)
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Manage Subjects</h1>
            ...
            {items.map((item, i) => (
              <tr key={item._id} className="border-b last:border-0 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-center">
                  ...
                  >
                    {item.is_active !== false ? <FiToggleRight /> : <FiToggleLeft />}
