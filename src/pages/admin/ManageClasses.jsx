...
export default function ManageClasses() {
  ...
  const fetchItems = async () => {
    try { const res = await getClasses(); setItems(res.data); }
  ...
  const handleCreate = async (e) => {
    ...
    try { await createClass({ name: newName.trim() }); setNewName(''); fetchItems(); }
    catch (err) { alert(err.response?.data?.message || 'Error'); }
  };

  const handleUpdate = async (id) => {
    if (!editName.trim()) return;
    try { await updateClass(id, { name: editName.trim() }); setEditId(null); fetchItems(); }
    catch (err) { alert(err.response?.data?.message || 'Error'); }
  };

  const handleDelete = async (id) => {
    if (!confirm('Delete this item?')) return;
    try { await deleteClass(id); fetchItems(); }
    catch (err) { alert(err.response?.data?.message || 'Error'); }
  };
  ...
  const handleToggleActive = async (id, currentStatus) => {
    try { 
      ...
      await updateClass(id, { is_active: newStatus }); 
      fetchItems(); 
    }
    catch (err) { alert(err.response?.data?.message || 'Error'); }
  };
